    ...
    <div className="space-y-4 p-4" data-testid="mcp-log-panel">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium">JSON-RPC Logs ({logs.length})</span>
        </div>
        {logs.length > 0 && (
      ...
      {logs.length > 0 ? (
        <div className="space-y-2">
          {logs.map((entry) => (
            <MCPLogEntryCard key={entry.id} entry={entry} isHighlighted={entry.id === highlightedLogId} />
          ))}
        </div>
      ) : (
        <div className="flex flex-col items-center justify-center py-8 text-center text-muted-foreground">
          ...
          <p className="text-xs">Connect and call a tool or prompt to see traffic</p>
        </div>
      )}
    </div>
  );
