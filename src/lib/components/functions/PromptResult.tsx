      ...
      {result.messages.map((msg, i) => (
        <div key={i} className="border rounded p-2 space-y-1">
          ...
          <div className="space-y-1">
            {(Array.isArray(msg.content) ? msg.content : [msg.content]).map((item, j) => {
              switch (item.type) {
                case "text": {
                  const trimmed = item.text.trim();
                  if (trimmed.startsWith("{") || trimmed.startsWith("[")) {
                    try {
                      return <JsonHighlight key={j} code={JSON.stringify(JSON.parse(trimmed), null, 2)} />;
                    } catch {
                      // Not JSON
                    }
                  }
                ...
                case "resource":
                  return (
                    <div key={j} className="text-xs font-mono bg-muted p-1 rounded">
                      {item.resource.uri}
                      {item.resource.text && <div className="mt-1 whitespace-pre-wrap">{item.resource.text}</div>}
                    </div>
                  );
                default:
                  return null;
