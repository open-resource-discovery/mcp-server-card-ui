...
const sortOrder: Record<ValidationStatus, number> = { fail: 0, warning: 1, pass: 2 };

export function ValidationPanel() {
  const { results, summary, isValidating, lastValidatedAt } = useValidationStore();
  const [activeFilter, setActiveFilter] = useState<ValidationStatus | null>(null);

  const allPassed = results.length > 0 && summary.fail === 0 && summary.warning === 0;

  const sortedAndFiltered = useMemo(() => {
    const sorted = [...results].sort((a, b) => sortOrder[a.status] - sortOrder[b.status]);
      ...
      {allPassed && !activeFilter ? (
        ...
        <div className="flex items-center justify-center py-8 text-center text-muted-foreground">
          <div>
            ...
            <p className="text-sm">Enter a server card JSON and validation will run automatically.</p>
          </div>
        </div>
      )}
    </div>
